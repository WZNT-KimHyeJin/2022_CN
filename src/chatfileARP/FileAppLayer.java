...
public class FileAppLayer implements BaseLayer {
    ...
    private String fileSaveDirectory = "." + File.separator;
    ...
    public synchronized boolean Receive(byte[] input) { // 데이터를 수신 처리 함수
        ...
        if(checkReceiveFileInfo(input)) { // 파일의 정보를 받은 경우
            ...
            file = new File(fileSaveDirectory + fileName); //받는 경로..
    ...
    public void setFileSaveDirectory(String newDirectory) {
    	this.fileSaveDirectory = newDirectory;
    }
    ...
    public boolean Send(byte[] input, int length) { // 데이터 송신 함수
        ...
        //((TCPLayer)this.GetUnderLayer()).fileSend(bytes, length + 12);
        return true;
    }
